        ...
        <form action="<?php echo $delete; ?>" method="post" enctype="multipart/form-data" id="form-author">
          <div class="table-responsive">
            <table class="table table-bordered table-hover">
              ...
              <tbody>
                <?php if (isset($author_groups)) { ?>
                <?php foreach ($author_groups as $author_group) { ?>
                <tr>
                  <td class="text-center"><?php if (in_array($author_group['author_group_id'], $selected)) { ?>
                    <input type="checkbox" name="selected[]" value="<?php echo $author_group['author_group_id']; ?>" checked="checked" />
                    <?php } else { ?>
                    <input type="checkbox" name="selected[]" value="<?php echo $author_group['author_group_id']; ?>" />
                    <?php } ?></td>
                  <td class="text-left"><?php echo $author_group['name']; ?></td>
                  <td class="text-right"><a href="<?php echo $author_group['edit']; ?>" data-toggle="tooltip" title="<?php echo $button_edit; ?>" class="btn btn-primary"><i class="fa fa-pencil"></i></a></td>
                </tr>
                <?php } ?>
                <?php } else { ?>
                <tr>
